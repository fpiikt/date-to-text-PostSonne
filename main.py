...
res1 = DateToTextClass()
assert res1.convert('25.09.2019 08:17:59') == "двадцать пятое сентября две тысячи девятнадцатого года восемь часов семнадцать минут пятьдесят девять секунд", 'ошибка в тестовом примере 1'
assert res1.convert('01.01.2001 01:01:01') == "первое января две тысячи первого года один час одна минута одна секунда", 'ошибка в тестовом примере 2'
assert res1.convert('06.12.1999 06:23:60') == "шестое декабря тысяча девятьсот девяносто девятого года шесть часов двадцать три минуты шестьдесят секунд", 'ошибка в тестовом примере 3'
assert res1.convert('04.06.2013 08:09:42') == "четвёртое июня две тысячи тринадцатого года восемь часов девять минут сорок две секунды", 'ошибка в тестовом примере 4'
assert res1.convert('08.09.2003 12:01:45') == "восьмое сентября две тысячи третьего года двенадцать часов одна минута сорокпять секунд", 'ошибка в тестовом примере 5'
